...
namespace UseCases.Test.UseCases.DailyCloses.Reopen;

public class ReopenDailyCloseUseCaseTest
{
    public static TheoryData<Role> ElevatedRoles => [Role.Manager, Role.Admin];

    [Theory]
    [MemberData(nameof(ElevatedRoles))]
    public async Task Execute_ShouldReopenApprovedCloseClearApprovalAndPreserveItems(Role role)
    {
        ...
        ctx.Close.ApprovedAt.ShouldBeNull();
        ctx.Close.ApprovedByUserId.ShouldBeNull();
        ctx.Close.ApprovedByUser.ShouldBeNull();
    ...
    public async Task Execute_ShouldRejectMember()
    {
        ...
        ctx.Close.Status.ShouldBe(DailyCloseStatus.Approved);
    ...
    private static ReopenDailyCloseUseCase CreateUseCase(TestContext ctx)
    {
        return new ReopenDailyCloseUseCase(
    ...
    private static TestContext BuildContext(Role role)
    {
        ...
        var item = new DailyCloseItemBuilder().WithValue(-18.25m).Build();
        var close = new DailyCloseBuilder()
            .WithStatus(DailyCloseStatus.Approved)
            .WithAccount(account)
            .WithDate(now.Date.AddDays(-1))
