...
public class TransactionControllerUnhappyPathTest(ServerWebApplicationFactory factory)
{
    ...
    [Fact]
    public async Task CreateInstallment_ShouldReturn403_BeforeNonChequeConflict_WhenMemberTargetsUnlinkedAccount()
    {
        // 403 (out-of-scope account) must be surfaced before the 409 non-cheque branch.
        var (user, branch, _, token) = await factory.SeedFullBranchContextAsync(
            "TxnInstallmentMember403", Role.Member);
        await factory.SeedOperatorAsync(branch.Id, userId: user.Id);
        var account = await factory.SeedAccountAsync(branch.Id, AccountType.Terminal);
        var category = await factory.SeedCategoryAsync(branch.Id, "Saídas", Direction.Out);
    ...
    {
        var (accountId, transactionTypeId, token) =
            await SeedInstallmentContextAsync(
                "TxnInstallmentSmallAuto",
        ...
        var request = new RequestCreateTransactionInstallmentJsonBuilder()
            ...
            .WithTransactionTypeId(transactionTypeId)
            .WithAccountId(accountId)
            .WithAutoGeneration(6, DateTime.Today.AddDays(30))
            .Build();
