...
- [ ] **4.1** Extend `IProductsRepository` with: `Add`, tracked `GetActiveByIdAndBranchId`, no-tracking variant, `ListActiveByBranchIdAsNoTracking(branchId)` (ordered by `DisplayOrder ASC, Id ASC`), `ExistsActiveByBranchIdAndName(branchId, name, exceptId?)`. (The existing M4 `ListActiveByIdsAndBranchIdAsNoTracking` stays — admin CRUD adds parallel methods, doesn't replace.)
- [ ] **4.2** Add Communication DTOs: `RequestCreateProductJson { string Name, int DisplayOrder }`, `RequestUpdateProductJson { string Name, int DisplayOrder }`, `ResponseProductJson`, `ResponseListProductsJson` envelope.
- [ ] **4.3** Add `CreateProductFluentValidation` and `UpdateProductFluentValidation`: non-empty `Name`, length ≤ 255, `DisplayOrder >= 0`. Shared rules in `ProductValidationExtensions.cs`.
- [ ] **4.4** Add the M6 resource keys needed by the Product slice: `PRODUCT_NOT_FOUND` ("Produto não encontrado"), `PRODUCT_ID_EMPTY` ("O identificador do produto deve ser informado"), `PRODUCT_NAME_REQUIRED`, `PRODUCT_NAME_MAX_LENGTH`, `PRODUCT_DISPLAY_ORDER_INVALID`, and `PRODUCT_SYSTEM_PROTECTED` ("Este produto é gerenciado pelo sistema e não pode ser renomeado ou desativado"). The `PRODUCT_NAME_CONFLICT` key was already added by Phase 1.
- [ ] **4.5** Implement `CreateProductUseCase`: authenticate → role check → validate → pre-check uniqueness → block creation of a product literally named "Diferença Caixa" (use the same name constant exposed by `ICashVarianceProductResolver` — do NOT hardcode the string in the use case). The system-only product is bootstrapped once by `CreateBranch`; admins must never create a second one. → `Add` → `Commit`.
- [ ] **4.6** Implement `ListProductsUseCase` and `GetProductUseCase` (any branch role).
- [ ] **4.7** Implement `UpdateProductUseCase`: authenticate → role check → validate → load tracked → 404 → resolve `cashVarianceProductId` via `ICashVarianceProductResolver.GetIdAsync(branchId)` → if `product.Id == cashVarianceProductId` and the payload Name differs from the system name, throw `OnValidationException(PRODUCT_SYSTEM_PROTECTED)` (400). `DisplayOrder` mutation IS allowed on the system product (admin can re-order the close form). → uniqueness pre-check excluding self → mutate → `Commit`.
- [ ] **4.8** Implement `DeactivateProductUseCase`: authenticate → role check → load tracked → 404 → resolve `cashVarianceProductId` → if `product.Id == cashVarianceProductId` throw `ConflictException(PRODUCT_SYSTEM_PROTECTED)` (409) → set `Active = false` → `Commit`.
- [ ] **4.9** Register the five new use cases (Create / List / Get / Update / Deactivate) in DI.
- [ ] **4.10** Add `ProductController`: `POST /product` (M/A), `GET /product` (any), `GET /product/{id:guid}` (any), `PUT /product/{id:guid}` (M/A), `DELETE /product/{id:guid}` (M/A). Full `[ProducesResponseType]` audit.
- [ ] **4.11** Add request builders + extend `ProductsRepositoryBuilder` (extending the existing M4 builder, not replacing it).
- [ ] **4.12** Add `Validators.Test` coverage for the two new validators.
- [ ] **4.13** Add `UseCases.Test` coverage: Create happy + role denial + 409 name conflict + 400 reserved-name "Diferença Caixa"; List/Get branch isolation; Update happy + 400 PRODUCT_SYSTEM_PROTECTED on attempted rename of system product + 200 allowed when only `DisplayOrder` changes on system product + 409 + 404; Deactivate happy + 409 PRODUCT_SYSTEM_PROTECTED on the system product + re-create-after-deactivate flow (new row, new `Id`, same name).
- [ ] **4.14** Add `WebApi.Test` happy + unhappy coverage; include a reload assertion that the `Diferença Caixa` row's `Name` is unchanged across an attempted rename HTTP request that returns 400, and that `DisplayOrder` is updated when only `DisplayOrder` changes on the system row.
