...
- [ ] **5.1** Confirm `ISettingsRepository` already exposes `GetByBranchIdAsNoTracking` (M3) and add `GetByBranchId(branchId)` if a tracked variant for update isn't already there. Read methods stay `AsNoTracking()` by default per the repo convention.
- [ ] **5.2** Add Communication DTOs: `RequestUpdateSettingJson { DateTime? LockDate, decimal? DailyTargetHours, decimal? LunchDeductionOver6H, decimal? LunchDeductionOver4H }` (all optional — admin can update one field at a time), `ResponseSettingJson` carrying the full shape. `GET /setting` returns the single row; `PUT /setting` performs the update.
- [ ] **5.3** Add `UpdateSettingFluentValidation`: `DailyTargetHours` if present must be `> 0` and `<= 24`; `LunchDeductionOver6H` if present must be `>= 0` and `<= 8`; `LunchDeductionOver4H` if present must be `>= 0` and `<= 8`; precision rules matching the persisted columns (`numeric(6,2)` and `numeric(4,2)`). LockDate validation lives in the use case because "no backwards move" requires the persisted value.
- [ ] **5.4** Add three new resource keys: `SETTING_LOCK_DATE_RETREAT` ("A data de bloqueio só pode avançar no tempo"), `SETTING_DAILY_TARGET_OUT_OF_RANGE`, `SETTING_LUNCH_DEDUCTION_OUT_OF_RANGE`.
- [ ] **5.5** Implement `GetSettingUseCase`: authenticate → any branch role → load `ISettingsRepository.GetByBranchIdAsNoTracking` → 500 `InvalidOperationException` if missing (the row is a `CreateBranch` invariant; a missing row is system bug, not user error — match the `Setting row missing for branch {branchId}` pattern from the TimeEntry calc service).
- [ ] **5.6** Implement `UpdateSettingUseCase`: authenticate → role check (Manager/Admin) → validate → load tracked → assert `LockDate` ratchet: if `request.LockDate is { } newLockDate && newLockDate < existing.LockDate`, throw `OnValidationException(SETTING_LOCK_DATE_RETREAT)` (400). → apply each non-null field with explicit assignment (no mapper, per the M3 simple-update rule) → `Commit` → return mapped response. The use case does NOT broadcast a "Setting changed" event; the M5 read paths already recompute on every read, so the next `TimeEntry` read picks up the new values automatically.
- [ ] **5.7** Register both use cases in DI.
- [ ] **5.8** Add `SettingController`: `GET /setting` (any branch role), `PUT /setting` (Manager/Admin). Full `[ProducesResponseType]` audit. The route is collection-level (no id in URL) because there's exactly one row per branch and the branch is resolved from the token.
- [ ] **5.9** Add `RequestUpdateSettingJsonBuilder`.
- [ ] **5.10** Add `Validators.Test` coverage for `UpdateSettingFluentValidation`: each rule one success + one failure.
- [ ] **5.11** Add `UseCases.Test` coverage: Get happy + 500 missing-setting invariant breach; Update happy with each field individually + role denial + LockDate ratchet rejection + LockDate equal-to-current accepted (no-op) + LockDate forward accepted.
- [ ] **5.12** Add `WebApi.Test` happy + unhappy. Include a reload assertion that confirms partial updates only touch the supplied fields (e.g. send only `DailyTargetHours`, assert `LunchDeductionOver6H` and `LockDate` are unchanged).
- [ ] **5.13** Add a regression test in `UseCases.Test/Services/TimeEntries/TimeEntryCalculationServiceTest.cs` or a dedicated integration test: after updating `LunchDeductionOver6H` via the new endpoint, a subsequent `GET /timeentry/{id}` for an in-progress current-day row reflects the new tier value in its live recompute. Proves the M5 read-time recompute path is genuinely live.
