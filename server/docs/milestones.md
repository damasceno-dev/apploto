...
- [ ] **4.13** Add `WebApi.Test` coverage for `401` unauthenticated, `403` unauthorized by branch role, `404` missing entity in branch scope, and `409` membership conflicts / last-admin violations, including the hard PostgreSQL uniqueness enforcement for `BranchUser (UserId, BranchId)`
...
- [ ] **4.15** Add architecture tests verifying all Milestone 1 use cases are explicitly registered in DI and all Milestone 1 controller endpoints declare explicit auth intent with `TokenAuthenticate` or `TokenAuthorize`
