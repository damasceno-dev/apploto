...
- [ ] **1.1** Update `server.Infrastructure/Configurations/CategoryConfiguration.cs`: change `HasIndex(c => new { c.BranchId, c.Name }).IsUnique()` to add `.HasDatabaseName("IX_Categories_BranchId_Name").HasFilter("\"Active\" = true")`. Use the explicit database name so `PostgresExceptionHandler` can key off it.
- [ ] **1.2** Update `server.Infrastructure/Configurations/TransactionTypeConfiguration.cs`: change `HasIndex(t => new { t.CategoryId, t.Name }).IsUnique()` to add `.HasDatabaseName("IX_TransactionTypes_CategoryId_Name").HasFilter("\"Active\" = true")`.
- [ ] **1.3** Update `server.Infrastructure/Configurations/ProductConfiguration.cs`: change `HasIndex(p => new { p.BranchId, p.Name }).IsUnique()` to add `.HasDatabaseName("IX_Products_BranchId_Name").HasFilter("\"Active\" = true")`.
- [ ] **1.4** Generate EF migration `LookupFilteredActiveUniqueIndexes` covering the three index changes. The migration must drop and re-create each index (PostgreSQL cannot add a filter to an existing UNIQUE index in place). Verify the generated `Up` and `Down` are clean (no extra column changes).
- [ ] **1.5** Extend `server.API/ExceptionHandling/PostgresExceptionHandler.UniqueConstraintMessages` with three new entries: `IX_Categories_BranchId_Name → CATEGORY_NAME_CONFLICT`, `IX_TransactionTypes_CategoryId_Name → TRANSACTION_TYPE_NAME_CONFLICT`, `IX_Products_BranchId_Name → PRODUCT_NAME_CONFLICT`. Add the three resource keys to `ResourcesErrorMessages.resx` and `.Designer.cs` with Portuguese text mirroring the `HOLIDAY_DATE_CONFLICT` / `CLIENT_CPF_CONFLICT` style. Naming follows the existing `TRANSACTION_TYPE_*` convention from M3 (underscore between `TRANSACTION` and `TYPE`).
- [ ] **1.6** Extend `PostgresExceptionHandlerTest` `KnownUniqueConstraintMessages` with the three new mappings.
- [ ] **1.7** Add three `tests/WebApi.Test/{Category,TransactionType,Product}/{Entity}NameUniquenessConstraintTest.cs` files mirroring `ClientCpfUniquenessConstraintTest` from Milestone 2: each test seeds two active rows with the same name and asserts the second insert throws `DbUpdateException` with `SqlState = "23505"` and the new constraint name. Also include a "same name with one row inactive should succeed" case to prove the filter is correctly applied.
