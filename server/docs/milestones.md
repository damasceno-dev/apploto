...
- [ ] **3.7** Add `GET /dailyclose/{dailyCloseId:guid}` and `GET /dailyclose` (with `[FromQuery] RequestListDailyClosesJson`) to `DailyCloseController`. Declare `[ProducesResponseType]` for 200/400/401/403/404 on Get and 200/400/401 on List *(Get endpoint done; List endpoint deferred to Slice 3b)*
- [ ] **3.8** Register `GetDailyCloseUseCase` and `ListDailyClosesUseCase` in `AppDependencyInjection` *(GetDailyCloseUseCase registered; ListDailyClosesUseCase deferred to Slice 3b)*
