...
- [ ] **3.1** Extend `ITransactionTypesRepository` with: `Add`, tracked `GetActiveByIdWithCategoryAndBranchId(id, branchId)` (returns the type with `Category` navigation populated and the cross-branch row already filtered out — branch scoping goes through `Category.BranchId`; the repository owns the `.Include` and the join), no-tracking variant, `ListActiveByBranchIdAsNoTracking(branchId)` (joins through `Category` for branch scoping), `ExistsActiveByCategoryIdAndName(categoryId, name, exceptId?)`. The branch scoping must always go through `Category.BranchId` — never assume a direct column.
- [ ] **3.2** Add Communication DTOs: `RequestCreateTransactionTypeJson { Guid CategoryId, string Name, SettlementRule SettlementRule, bool RequiresTabAccountAndClient }`, `RequestUpdateTransactionTypeJson { string Name, SettlementRule SettlementRule, bool RequiresTabAccountAndClient }` (CategoryId NOT in Update DTO — explicit immutability), `ResponseTransactionTypeJson` carrying full shape including `CategoryId` and `CategoryName`, `ResponseListTransactionTypesJson` envelope.
- [ ] **3.3** Add `CreateTransactionTypeFluentValidation` covering non-empty `CategoryId`, non-empty `Name`, length, enum validity for `SettlementRule`. Add `UpdateTransactionTypeFluentValidation` covering the editable subset. Shared rules in `TransactionTypeValidationExtensions.cs`.
- [ ] **3.4** Add the M6 resource keys needed by the TransactionType slice: `TRANSACTION_TYPE_NOT_FOUND` ("Tipo de transação não encontrado" — verify whether this already exists from M3 and reuse if present; only add if missing), `TRANSACTION_TYPE_ID_EMPTY` ("O identificador do tipo de transação deve ser informado" — also possibly existing from M3), `TRANSACTION_TYPE_NAME_REQUIRED`, `TRANSACTION_TYPE_NAME_MAX_LENGTH`, `TRANSACTION_TYPE_CATEGORY_ID_EMPTY`, `TRANSACTION_TYPE_SETTLEMENT_RULE_INVALID`. The `TRANSACTION_TYPE_NAME_CONFLICT` key was already added by Phase 1. Naming follows the existing `TRANSACTION_TYPE_*` convention with the underscore.
- [ ] **3.5** Implement `CreateTransactionTypeUseCase`: authenticate → role check → validate → load `Category` tracked (or use the existing `GetActiveByIdAndBranchId` variant) and assert `Category.BranchId == branchUser.BranchId` (404 with `CATEGORY_NOT_FOUND` if not) → uniqueness pre-check via `ExistsActiveByCategoryIdAndName` → `Add` → `Commit` → mapped response with `CategoryName`.
- [ ] **3.6** Implement `ListTransactionTypesUseCase` (any branch role) and `GetTransactionTypeUseCase` (any branch role, 404 on missing or cross-branch).
- [ ] **3.7** Implement `UpdateTransactionTypeUseCase`: authenticate → role check → validate → load tracked via `GetActiveByIdWithCategoryAndBranchId` → 404 if missing or cross-branch → uniqueness pre-check excluding self → mutate `Name`, `SettlementRule`, `RequiresTabAccountAndClient` → `Commit`. Document explicitly in the use-case XML doc summary that changes are forward-only: existing `Transaction` rows are NOT recomputed (they snapshotted `Direction`, `CategoryId`, and `DueDate` at create time per §6.1).
- [ ] **3.8** Implement `DeactivateTransactionTypeUseCase`: authenticate → role check → load tracked → 404 → set `Active = false` → `Commit`. No cascade — Transactions reference TransactionType by id but a soft-deactivated type does not invalidate historical rows.
- [ ] **3.9** Register the five new use cases (Create / List / Get / Update / Deactivate) in DI.
- [ ] **3.10** Add `TransactionTypeController`: `POST /transactiontype` (M/A), `GET /transactiontype` (any), `GET /transactiontype/{id:guid}` (any), `PUT /transactiontype/{id:guid}` (M/A), `DELETE /transactiontype/{id:guid}` (M/A). Full `[ProducesResponseType]` audit.
- [ ] **3.11** Add request builders + extend `TransactionTypesRepositoryBuilder`.
- [ ] **3.12** Add `Validators.Test` coverage for the two new validators (success + one failure per rule).
- [ ] **3.13** Add `UseCases.Test` coverage: Create happy + role denial + 404 category-not-found + 404 cross-branch category + 409 name conflict in same category + 200 same name allowed in different category; List + Get branch isolation; Update happy + 409 + 404 cross-branch + forward-only assertion (an existing `Transaction` row's denormalized `Direction` and `CategoryId` are unchanged after the parent TransactionType's metadata is updated — assert via a small mock-based test); Deactivate happy + re-create-after-deactivate flow.
- [ ] **3.14** Add `WebApi.Test` happy + unhappy path classes covering the same matrix end-to-end with reload-based persistence verification.
