...
- [ ] **6.5.7** Wire `DailyCloseWorkflowGuard.EnsureCanOpen` explicitly instead of leaving it as a no-op. Preserve the existing Open use-case enforcement order (validation → account branch lookup → `MemberAccountScopeGuard` → workflow guard → lock-date → unique constraint), document which Open rules belong outside the guard (account scope, duplicate close race), and add workflow-guard tests proving Manager/Admin are allowed, Member without a linked operator is rejected, and linked Member happy path still delegates cleanly from `OpenDailyCloseUseCase`
