...
Refactor `TimeEntry` from a single `(ClockIn, ClockOut)` pair into a parent + N `TimeEntrySegment` child rows so a member can clock out mid-day for an errand and clock back in without losing or overstating hours. `PUT /timeentry` uses a **dual-shape contract**: Member callers send `Action: "Open" | "Close"` (no `Segments` field) — the server applies the action against the parent's current state and stamps the time itself, with `Open`-when-already-open and `Close`-when-already-closed both treated as idempotent no-ops; Admin/Manager callers send an explicit `Segments` snapshot for surgical control (no `Action` field). New admin granular endpoints (`POST/PUT/DELETE` on segments) cover back-fill and per-segment edits. This phase lands **before** Phase 4 so the read path is built segment-aware from day one.
...
- [x] **3.6.5** EF migration `AddTimeEntrySegments`:
  ...
  - **This migration does NOT touch existing `TimeEntries` rows or columns** — backfill and column drops are owned by Slice 3.6.B's separate migration.
...
- [ ] **3.6.10** Refactor `server.Application/Services/TimeEntries/ITimeEntryCalculationService.cs` to a single `Calculate(TimeEntryStatus status, IReadOnlyList<TimeEntrySegmentInput> segments, DateTime entryDate, DateTime branchLocalNow, decimal dailyTargetHours, decimal lunchDeductionOver6H, decimal lunchDeductionOver4H)` where `TimeEntrySegmentInput` is a `readonly record struct (DateTime ClockIn, DateTime? ClockOut)`. Implement the top-up gap-aware lunch rule in `TimeEntryCalculationService.cs` using direct DateTime subtraction (no 24h wrap-around branch — DateTime makes overnight unambiguous). Remove the old `Calculate(..., clockIn, clockOut, ...)` and `CalculateLiveRunning(...)` signatures.
- [ ] **3.6.11** Rewrite `tests/UseCases.Test/Services/TimeEntries/TimeEntryCalculationServiceTest.cs`:
...
- [ ] **3.6.12** Request DTOs in `server.Communication/Requests/`:
...
- [ ] **3.6.13** Domain: drop `ClockIn` and `ClockOut` properties from `server.Domain/Entities/TimeEntry.cs`. Update `server.Infrastructure/Configurations/TimeEntryConfiguration.cs` to drop the `ClockIn`/`ClockOut` property maps. Update `ITimeEntriesRepository.GetByBranchIdOperatorIdAndDate` to `.Include(t => t.Segments.Where(s => s.Active))` with deterministic segment ordering by `ClockIn ASC, CreatedAt ASC, Id ASC`. EF migration `MigrateTimeEntryClocksToSegments`:
  - Because the Milestone 5 database is still empty/non-production, this migration does not backfill existing TimeEntry rows. It only performs the schema transition: create TimeEntrySegments, drop TimeEntries.ClockIn/ClockOut, and update the model snapshot.
  - Drop `ClockIn`, `ClockOut` columns from `TimeEntries`.
  - `Down` reverses cleanly for local dev resets (re-add columns; copy back the first active segment's wall-clock components, collapsing any overnight wrap to `.TimeOfDay`; drop the new table rows).
- [ ] **3.6.14** Refactor `server.Application/UseCases/TimeEntries/Upsert/UpsertTimeEntryUseCase.cs` to the dual-shape contract:
  ...
  - Admin back-fill with explicit `ClockIn` (and optional `ClockOut`) and `Segments` populated → succeeds.
  ...
  - Overnight: Admin back-fill with `{2026-05-08T22:00, 2026-05-09T06:00}` (single segment crossing midnight) succeeds.
- [ ] **3.6.15** Remove the obsolete resource keys from `server.Exceptions/ResourcesErrorMessages.resx` and `.Designer.cs`: `TIMEENTRY_NON_PRESENT_REJECTS_CLOCKS` (replaced by `..._SEGMENTS`), `TIMEENTRY_PRESENT_REQUIRES_CLOCK_IN` (Member doesn't supply clocks anymore), `TIMEENTRY_CLOCK_OUT_REQUIRES_CLOCK_IN` (clock pairs live on segments only). Rewrite `tests/WebApi.Test/TimeEntries/TimeEntryControllerUpsertHappyPathTest.cs` and `TimeEntryControllerUpsertUnhappyPathTest.cs` for the dual-shape contract:
  - **Happy paths:** Member tap E2E with explicit `Action`: `Open` → `Close` → `Open` → `Close`, all with `Segments` omitted; assert response `Segments` and `IsInProgress` flip correctly; reload-based confirmation (segment ids stable on close). Member retry idempotency: a duplicate `Action = Open` PUT against an already-open parent returns 200 with the unchanged state (no new segment); a duplicate `Action = Close` PUT against an already-closed parent likewise returns 200 unchanged. Admin explicit-times back-fill happy path with `Segments` populated; multi-segment Admin upsert; single-segment overnight Admin back-fill with reload confirming `ClockOut` on the next calendar day.
...
- [ ] **4.2** Add lightweight list item response carrying `TimeEntry` summary, `OperatorName`, and the `IsInProgress` flag (`true` iff any active segment has `ClockOut == null`). The summary's `TotalHours` / `BalanceHours` reflect the read-time live-running recompute described above and in 4.5 / 4.6, not the persisted snapshot. The response shape carries `Segments` only on the Get endpoint; the list item omits per-segment detail by default.
