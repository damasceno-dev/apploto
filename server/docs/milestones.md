...
Final pass to close Milestone 6 with the same discipline used in M5 Phase 6.

- [ ] **7.1** Audit `[ProducesResponseType]` declarations on `CategoryController`, `TransactionTypeController`, `ProductController`, `SettingController`, and the two new `HolidayController` actions. Each action declares **exactly** the status codes it can return — no missing codes, no spurious ones. The `WebApi.Test` suite for each endpoint must hit every declared code at least once across happy/unhappy tests.
- [ ] **7.2** Confirm the architecture-test suite picks up every new M6 class. The reflection-based `AllUseCases_AreRegisteredInApplicationDi` and `AllPublicServicesAndExceptionHandlers_AreResolvableFromConfiguredRootContainer` should cover the new use cases (Categories/{Create,List,Get,Update,Deactivate}, TransactionTypes/* (5), Products/* (5), Settings/{Get,Update}, Holidays/{PreviewBrazilianImport,ImportBrazilian}) and the new `IBrazilianHolidayCalendar` automatically. Add an explicit resolution test only if a gap surfaces.
- [ ] **7.3** Confirm validator, use-case, Web API, and architecture tests cover every new endpoint and service introduced in M6 — Categories, TransactionTypes, Products, Setting, and Brazilian Holiday import. For any gap, add the missing test with a "Phase 7 catch-up" comment.
- [ ] **7.4** Spec sync: update `loto-specs.md` to reflect the filtered-unique change introduced by Phase 1 — change §3.9 line ~407 from `**Unique constraint:** (BranchId, Name)` to `**Unique constraint:** filtered unique (BranchId, Name) WHERE Active = true`; same shape for §3.10 line ~440 (`(CategoryId, Name) WHERE Active = true`) and §3.13 line ~619 (`(BranchId, Name) WHERE Active = true`). Add a v21 entry to the revision notes recording the filtered-unique migration and the new §5.1 Brazilian Holiday Calendar appendix (the appendix itself ships in Phase 6 item 6.12). Verify the new §5.1 added by Phase 6 is in place. Update `loto_presentation.html` and `loto_entity_relationship_diagram.html` if any factual content references the affected sections (the ERD does not display index filters — likely no change needed). Bump shared `Spec revision` to v21 across all three files. Run `bash server/docs/check-loto-doc-sync.sh` and confirm green.
- [ ] **7.5** Run `dotnet test tests/Validators.Test`.
- [ ] **7.6** Run `dotnet test tests/UseCases.Test`.
- [ ] **7.7** Run `dotnet test tests/WebApi.Test`.
- [ ] **7.8** Update this milestone checklist with completion notes where implementation intentionally differs from the original plan.
...
- `IBrazilianHolidayCalendar` returns the canonical Brazilian calendar for a given year — 10 mandatory national holidays (by federal law) plus, when `includeOptionalFederal = true`, a curated 3-entry operational subset of the MGI annual optional federal calendar (Carnaval terça, Quarta-feira de Cinzas até 14h, Corpus Christi — the three most consistently observed by businesses; other MGI optionals like Carnival Monday, the Corpus Christi bridge Friday, or the eve-day afternoons remain available via the existing per-branch `POST /holiday`). `GET /holiday/import-br/{year}/preview?includeOptionalFederal=true|false` exposes the list (10 or 13 entries) with `AlreadyExists` flags so the admin UI can show a confirmation. `POST /holiday/import-br/{year}?includeOptionalFederal=true|false` (Manager/Admin) inserts the missing entries into the branch's `Holiday` table via the existing batch path, returning per-item `Imported` / `Skipped` status. The import is idempotent: running it twice on the same year+flag is a no-op on the second call.
- Read endpoints on `Category`, `TransactionType`, `Product`, and `Setting` are open to any authenticated branch role; write endpoints are Manager/Admin only. Architecture tests confirm explicit auth intent across all new endpoints.
- Validator, use-case, Web API, and architecture tests pass with the new admin CRUD surfaces and the Brazilian holiday import. `check-loto-doc-sync.sh` passes. Spec revision is bumped if any sync-group file changes (the Brazilian holiday appendix in §5.1 is the most likely trigger).
