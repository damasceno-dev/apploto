...
> **Status:** Revised spec (v30) — Milestone 7 close-out (Phase 12). The §6.14 reporting contract is **shape-stable** for downstream code generation — endpoints, response DTOs, status codes, and semantics are locked — and the preview-never-commits invariant is pinned at the architecture level (a reflection scan fails the build if any report/preview use case injects `IUnitOfWork`). One caveat is tracked separately: project-wide OpenAPI `required`-metadata emission is still pending (moved out of this milestone into the new **Backend OpenAPI Contract Hardening** milestone), so generated TypeScript currently treats fields as optional until that lands. §6.14 gains an explicit "Time-entry balance summary" subsection (branch-wide all-operator roll-up + `Operators` wrapper, Phase 11 addendum). One contract correction lands in this revision: `GET /report/monthly-reconciliation/{year}/{month}` now returns a real 400 `ResponseErrorJson` for an out-of-range year/month (the route dropped its `min/max` constraints so `MonthlyReconciliationFluentValidation` owns the range), instead of a body-less framework 404 — making its declared `{200,400,401,403}` contract fully reachable and testable. All eleven M7 endpoints (eight `GET /report/*`, three `POST …/preview`) are live, read-only, and audited.
