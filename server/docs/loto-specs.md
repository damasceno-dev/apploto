...
> **Status:** Revised spec (v26) — Milestone 7 Slice 7 Edit Impact Preview — §6.14 "Edit impact preview" subsection added covering `POST /transaction/{id}/edit-preview` (Manager/Admin), the `RequestUpdateTransactionJson` payload reuse, `asOfDate` query parameter, the three always-present impact sections (receivable / fiado-balance / cash-variance with a live-recomputed `CurrentVariance` for any non-Draft close, the close status surfaced via `DailyCloseStatus`, and a computed net-flow delta), the preview-never-commits invariant pinned by reload assertions plus the determinism test, and monthly lock-readiness deferred to the Milestone 10 monthly-reconciliation report
...
*Response (`ResponseEditTransactionPreviewJson`):* `{ TransactionId, Impact, Warnings }`. `Warnings` is currently always empty. `Impact` (`ResponseTransactionEditImpactJson`) always carries all three sections; each is "empty" when not relevant:
...
*Preview-never-commits invariant:* the use case takes no `IUnitOfWork` dependency, so it cannot persist by construction. This is pinned two ways — the Phase 12 architecture test scans the constructor for any `IUnitOfWork` parameter, and the WebApi tests reload the row after a 200 response and assert it is byte-for-byte unchanged. A determinism test additionally ties the predicted `ReceivableImpact.BucketAfter` to the row's actual `Bucket` in `GET /report/fiado/aging` after the same payload is committed via `PUT /transaction/{id}`.
