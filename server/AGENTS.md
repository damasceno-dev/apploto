...
- Treat `server/docs/loto-specs.md`, `server/docs/loto_presentation.html`, and `server/docs/loto_entity_relationship_diagram.html` as one Lotero backend doc sync group. Factual changes must update the affected files together, keep the shared sync metadata aligned, and pass `server/docs/check-loto-doc-sync.sh`.
