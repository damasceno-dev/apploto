...
- Declared public enum values serialize with their canonical declared casing. If legacy or out-of-band data surfaces an undefined response value, serialize its numeric backing value rather than throwing during result execution and risking a truncated response. During the transition window, request JSON accepts declared names case-insensitively (matching MVC query binding) and any integer representable by the enum's backing type; responses always normalize a recognized name back to its canonical declared casing. Feature validators own declared-member checks and their localized messages, while invalid or case-insensitively ambiguous names and numerically unrepresentable values fail at binding. Every request enum property, including nullable properties, must have a matching FluentValidation `IsInEnum` rule. All model-state failures use the standard `ResponseErrorJson` 400 envelope, and only explicitly allowlisted backend-authored messages may escape the binding boundary. OpenAPI enum schemas use those same canonical declared names as `type: string`, including enums reached through nullable-only properties and query/body contracts; never maintain a parallel enum-name list.
...
- Every controller action declares its returnable status codes and response types with `[ProducesResponseType]`. An action that returns `IActionResult` without them is invisible to code generation: its response DTO never reaches `components/schemas` at all.
- The whole generated document is pinned by a contract-surface snapshot: `tests/WebApi.Test/OpenApi/Snapshots/openapi-contract-surface.txt`, produced by `OpenApiContractSurface.Render` and asserted by `OpenApiContractSurfaceSnapshotTest`. Keep that projection deterministic — no timestamps, no host URLs, no unsorted collections, no generator noise. When a contract change is intended, run the test, review the emitted `openapi-contract-surface.received.txt`, and replace the expected file with it; never regenerate it to make a red build green. The snapshot-independent assertions in the same class (no numeric enum anywhere, every contract enum a named string component, every response-reachable enum value serialized by name) must keep passing on their own.
