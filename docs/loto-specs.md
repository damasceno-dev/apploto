...
> **Status:** Revised spec (v31) — Milestone 7.5 close-out (Frontend UX Contract Gaps). §6.14 gains the two frontend-driven read contracts surfaced by the M0 screen catalog: `GET /report/dashboard?date=` (manager work-queue aggregation — per-close variance joined by `(Date, AccountId)`, pending-approval count, expected-closer/not-submitted rows with Draft deep-links, branch-level variance aggregates) and `GET /dailyclose/{dailyCloseId}/review` (close header + per-product opening → closing context with §6.5 server-derived opening values and the system-calculated `Diferença Caixa` row flagged). The M7.5 Phase 3 list-variance gate resolved as a **deferral** — `ResponseListDailyCloseItemJson` is intentionally unchanged. Both new endpoints are read-only; the architecture-level never-commits scan now also covers the daily-close review namespace. No schema changes. The v30 caveat still stands: project-wide OpenAPI `required`-metadata emission is tracked in the **Backend OpenAPI Contract Hardening** milestone, so generated TypeScript treats fields as optional until that lands.
...
**Read-only contract.** All Milestone 7 and Milestone 7.5 reporting endpoints are read-only. None call `Add`, none mutate persisted state, none open a unit of work. The three preview endpoints (`POST /transaction/installment/preview`, `POST /transaction/{id}/edit-preview`, `POST /transaction/preview`) are compute-only and never persist; the "preview cannot commit" invariant is enforced by construction (no `IUnitOfWork` dependency) and pinned by WebApi.Test reload / row-count assertions that verify persisted state is unchanged after a 200 response. The architecture-level never-commits scan covers the report namespaces and, since the M7.5 close-out, the daily-close review context (`UseCases.DailyCloses.Review`) as well.
...
**Daily-close review context.** `GET /dailyclose/{dailyCloseId}/review` — any branch role within the DailyClose read scope (M7.5 Phase 2). Returns the close header plus per-product opening → closing context for the close-day form and the manager approval review, with opening values derived server-side per §6.5 instead of making clients rediscover the prior close and recompute the mapping.
...
*Response (`ResponseDailyCloseReviewJson`):* a dedicated review DTO — the header fields mirror `ResponseDailyCloseJson` (id, date, status, account, submitted/approved audit pair, rejection reason, notes, update audit) while `Items` is review-specific. It is intentionally a separate shape: `ResponseDailyCloseJson` is also returned by write operations, where computing review-only opening values would make the shared contract context-dependent.

*Per-item contract (`ResponseDailyCloseReviewItemJson`):* `{ ProductId, ProductName, OpeningValue?, ClosingValue, IsCashVarianceProduct }`. Active items only, ordered `ProductName ASC (ordinal), ProductId ASC`.

*Opening-value projection (§6.5):* the opening source is the most recent prior active close for the same `(BranchId, AccountId)`, ordered `Date DESC, CreatedAt DESC, Id DESC` — weekends and holidays are skipped naturally by most-recent-prior ordering. Prior active items are mapped by `ProductId`; a product with no prior item (or no prior close at all) gets `OpeningValue = 0`. Products that existed only on the prior close are not resurrected — the item list is driven by the current close. The `"Diferença Caixa"` row is flagged `IsCashVarianceProduct = true` (ordinal name match) and its `OpeningValue` is always `null`: it is a system-calculated §6.12 output, not an operator-entered count, so a prior variance is never presented as an opening product value.
