...
> **Status:** Revised spec (v32) — CashVariance Direction-semantics clarification. §6.12 now documents what `In`/`Out` mean on a Terminal account — value entering/leaving the operator's custody, verified against the original Access `FrmCaixa` conferência and its production data — pinning the formula's sign convention, which is confirmed correct and unchanged; Access-faithful regression tests now guard it in UseCases.Test and WebApi.Test. No contract or schema changes: the v31 M7.5 read contracts (`GET /report/dashboard`, `GET /dailyclose/{dailyCloseId}/review`, Phase 3 list-variance deferral) are unchanged, and the v30 caveat still stands — project-wide OpenAPI `required`-metadata emission is tracked in the **Backend OpenAPI Contract Hardening** milestone, so generated TypeScript treats fields as optional until that lands.
