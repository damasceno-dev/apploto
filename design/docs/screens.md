...
These are the remaining open entries after M7.5 and M7.7 Phases 2–4, 6, and 7. B, E, G, I, K, M, O, and T no longer gate M4; L remains only for its Phase 5 workstation/paired-Tab routing and assignment UX. None blocks M1 (visual direction), but **every screen named in the remaining map below is implementation-blocked for M4 blueprints and the build**. Decisions land through [server M7.7](../../server/docs/milestones.md) (M9.5 verification feeds the fiado one; server M8 owns C/N; design 0.4.5 owns P; design 0.4.1 owns R; server M11 owns Q); the catalog entries are re-synced when their owning implementation lands.
