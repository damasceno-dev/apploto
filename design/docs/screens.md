...
> **Status:** Draft (Design M0, Phase 3 complete). Template + all screens defined (0.3.3 done); the navigation map and cross-screen patterns land in Phase 4, which also locks this catalog.
...
All screens are defined, ordered by area exactly as listed below; the five **spine screens** (0.3.2) are marked where they appear.
...
- **Data shown** *(a queue grouped by exception type, most-urgent first; the close, variance, and not-submitted groups all come from **one call** — `GET /report/dashboard?date=`)*
  ...
  - **Draft transactions blocking month lock** — count of `Status = Draft` transactions in the open period. `[composed]`: `GET /transaction` filtered to `Status = Draft` (also surfaced as a reconciliation blocker).
...
- **States**
  ...
  - *empty* — nothing needs action → an all-clear state. (The dashboard response covers only the selected date; a recent-close summary, if shown, is `[composed]`: `GET /dailyclose` most-recent row.)
...
- **Audit / lock context** — blocked when `Date ≤ LockDate`; `RecordedByOperatorId` stamped; Draft plans stay out of the open-cheque report, fiado sums, and variance until finalized.
...
- **Actions** — filter one client / change the as-of date → `GET /report/fiado/balance`; drill a client → Fiado aging with the client filter carried.
...
- **Actions** — filter client/account/as-of → `GET /report/fiado/aging`; open a row → Transaction — Edit (typically to set `PaidAt` when the client settles).
...
- **Data shown**
  - **Lock readiness** — `LockReady` (true only when every active close is Approved and zero Draft transactions remain). `[DTO: ResponseMonthlyReconciliationJson]`.
...
- **Access** — any branch role (`[TokenAuthenticateBranch]`): Member → own operator; Manager/Admin → `operatorId` (one), `mine` (own), or **neither → branch-wide roll-up** (one element per operator with entries in the window — the payroll view).
...
- **Navigation** — operator home after branch session; links to Transaction — Create (fast entry), Open day, Close day, Fix & resubmit, Clock in / out, My transaction summary, and My time-entry balance.
...
- **Data shown**
  ...
  - **Variance row** — flagged `IsCashVarianceProduct = true` on review items; it **exists only after the first submit** (the system writes it then, updating it in place on resubmits). While editing a never-submitted close there is no variance row to show — the comparison context appears on review after submit. `[DTO]`.
...
- **Access** — the same edit-items matrix as Close day. **Same-local-day caveat:** the recording Member can edit a `Rejected` close only on the same branch-local day (§6.13 — "same rules as Draft"); after that day, a Manager/Admin must make the correction.
- **Data shown**
...
- **Default view / filter** — current month, all operators, with `IsInProgress` rows surfaced (forgotten punches are the top correction target).
- **Data shown**
...
- **Data shown** — `LockDate`, `DailyTargetHours`, `LunchDeductionOver6H`, `LunchDeductionOver4H` — decimal hours render as durations (formatting §5: `7.33` → `7h 20min`). `[DTO: ResponseSettingJson]`.
- **Actions** — update → `PUT /setting` (partial: only provided fields change; `[DTO: RequestUpdateSettingJson]`). **`LockDate` only advances** — a backward move is rejected (verbatim); advancing it normally happens from Monthly reconciliation, which uses this same endpoint.
