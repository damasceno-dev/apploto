...
> **Status:** Draft (Design M0, Phase 3 complete). Template + all currently-cataloged screens defined (0.3.3 done); the navigation map and cross-screen patterns land in Phase 4, which closes this catalog as a **reviewed baseline** — not a final lock: every screen named in the gaps section's **gap → screen map** is **decision-blocked** until its decision lands and the entry is re-synced (inline pointers are mirrors, the map is authoritative). The 2026-07 catalog review added gaps **F–T** (domain decisions pending, mostly server-side — see the gaps section).
...
- **Data shown** *(comparison: opening → closing → variance, with source clearly marked)*
  - **Close header** — account, date, status, `SubmittedByOperatorName?`, `SubmittedAt?` (both nullable), rejection reason (if any). `[DTO: ResponseDailyCloseReviewJson]` — **one call**, `GET /dailyclose/{id}/review`, serves the header and every item row below.
...
- **Audit / lock context** — submitted-by/at, approved-by/at, rejection reason; if already `Approved`/`Rejected`, the screen is read-only. *(Gap §0.3.3-G: the persisted variance can be stale — ledger mutations after submit don't touch the close. Gap §0.3.3-J: `DailyClose.Notes` exists in the contract but no flow lets the operator record the variance cause this screen is meant to review.)*
- **States**
...
- **Audit / lock context** — blocked when `Date ≤ LockDate` (explain the period is locked); `RecordedByOperatorId` stamped. *(Gap §0.3.3-G: creating rows under a Submitted/Approved close follows the chosen close-integrity rule; financial creates also gain idempotency keys — server M7.7 Phase 6.)*
...
- **Audit / lock context** — blocked when `Date ≤ LockDate`; `RecordedByOperatorId` stamped; Draft plans stay out of the open-cheque report, fiado sums, and variance until finalized. *(Gaps §0.3.3-G/M: plan creation under a Submitted/Approved close follows the chosen close-integrity rule; plan cancellation lacks the sibling lookup.)*
...
- **Data shown**
  - **Rows** — `Date`, `AccountName`, `VarianceValue` (signed, formatting §3), `DailyCloseStatus` (a pending number reads differently from a signed-off one). `[DTO: ResponseCashVarianceSummaryItemJson]`.
...
- **Data shown**
  ...
  - **Calendar days** — per day: closes (`AccountName`, `Status`, `VarianceValue`, `DailyCloseId`), `ActiveTransactionCount`, `DraftTransactionCount`, `CancelledTransactionCount`, `NetVariance`. `[DTO: ResponseMonthlyReconciliationDayJson / ...DayCloseJson]`.
...
- **Data shown** *(next-action oriented)*
  - **Today's close state** — is there an open `Draft` close? its status? `[composed]`: `GET /dailyclose` filtered to `(account, today)`; the list item carries no `RejectionReason`, so a rejected close's reason needs `GET /dailyclose/{id}`.
  ...
  - **Next action** — `[derived]`: no close → open day; open → record / close day; submitted → awaiting approval; rejected → fix & resubmit.
...
- **Navigation** — operator home after branch session; links to Transaction — Create (fast entry), Open day, Close day, Fix & resubmit, Clock in / out, My transaction summary, and My time-entry balance. *(Gaps §0.3.3-I/L/R shape this screen: the rejected-flow edit window, workstation-first framing + multi-terminal selector, and the manager-as-operator mode.)*

---

## Open day

- **Purpose** — create today's `Draft` close for the operator's account — the flag that the drawer is being tracked.
- **Primary job** — one tap: confirm the account, open the day.
- **Access** — Member with linked operator + account in scope; Manager/Admin may open for any account (§6.13 Open matrix). *(Gap §0.3.3-L: the server does not restrict opens to Terminal accounts, though only terminals are expected closers — a close on a Tab/Bank account is currently possible and meaningless.)*
- **Permission fallback** — Member without a linked operator → 403 (verbatim, `EnsureCanOpen`); account out of scope → 403.
- **Default view / filter** — today (branch-local, `[derived]` from the client clock — self-context carries no date; server M7.7 Phase 6 ships a server-authoritative branch date/time in the session/context reads, after which this client-clock derivation is replaced) + the operator's primary account `[DTO: GET /operator/self-context → ResponseSelfContextJson.PrimaryAccount?]`. **`PrimaryAccount` is nullable** — a linked operator may have accounts but no primary: fall back to an explicit pick from `AvailableAccounts` (and to the setup-needed state when that list is empty too).
...
- **Audit / lock context** — `LockDate` guard applies (rare for today); a duplicate open close for `(account, date)` → conflict verbatim (unique-constraint translation); the UI then resolves the existing close `[composed]`: `GET /dailyclose?AccountId&DateFrom=DateTo=` today, and jumps to it.
...
- **Access** — the §6.13 edit-items matrix: the Member who recorded the close, on the same branch-local day; Manager/Admin anytime (pre-lock).
...
- **Data shown**
  - **Product rows** — the form enumerates **active products** ordered by `DisplayOrder`. `[composed]`: `GET /product` — the review endpoint cannot serve this: it projects only the close's *existing* items. **Exclude the system row**: `GET /product` includes "Diferença Caixa" and carries no system flag, and sending it in items → 400 `DAILYCLOSE_ITEM_PRODUCT_FORBIDDEN` — so the form filters it by exact name (`[derived]` — acceptable because the server guards the reserved name, §6.13 / `PRODUCT_SYSTEM_PROTECTED`); a proper server-provided flag ships with the gap-D review expansion.
  - **Opening values** — `OpeningValue?` per item (§6.5 server-derived; zero when no prior item exists) from `[DTO: GET /dailyclose/{id}/review → ResponseDailyCloseReviewItemJson]` — the same endpoint the manager's review uses; a Member within account scope may call it. **Caveat: a fresh, just-opened close has no items, so review returns no rows** — the data is then reachable only by re-deriving the §6.5 prior-close lookup client-side, which the catalog deliberately avoids (the selection rules would be duplicated in web and mobile). Open `[gap]` §0.3.3-D documents the interim composition and the preferred server fix.
  - **Variance row** — flagged `IsCashVarianceProduct = true` on review items; it **exists only after the first submit** (the system writes it then, updating it in place on resubmits). While editing a never-submitted close there is no variance row to show — the comparison context appears on review after submit. `[DTO]`. **Gap §0.3.3-F:** §7.1 expects the variance visible *while counting* (the legacy Access recalculated live); a pre-submit server-side variance preview is pending — without it the operator submits blind and the recall loop becomes the de facto workflow.
  - **Item form** — `{ProductId, Value}` per editable product. `[DTO: RequestPutDailyCloseItemsJson / RequestUpsertDailyCloseItemJson]` — sending the variance product is rejected (`DAILYCLOSE_ITEM_PRODUCT_FORBIDDEN`).
  - **Draft-transaction warning** — count of today's `Draft` rows on this account (drafts don't count in the variance). `[composed]`: `/transaction` filtered `(account, today, Draft)`.
...
- **Actions** — save values → `PUT /dailyclose/{id}/items` (a `Rejected` close auto-transitions to `Draft`; a same-day `Submitted` close is recalled to `Draft` — §6.5); **Submit** → `POST /dailyclose/{id}/submit` (the system computes and persists the variance item).
- **Audit / lock context** — every transition stamps `UpdatedAt`/`UpdatedByUserId`; submit stamps `SubmittedAt`; all transitions blocked at/before `LockDate`.
- **States** — *loading* (review context) · *validating* · *lock-blocked* · *submitted* (read-only, awaiting review) · *success* · *error*.
- **Navigation** — from the Day Cockpit; after submit, back to the cockpit's awaiting-approval state.

## Fix & resubmit

- **Purpose** — the recovery path after a manager rejection: see why, correct the values, send again.
- **Primary job** — read the rejection reason and resubmit a corrected close without hunting for what changed.
- **Access** — the same edit-items matrix as Close day. **Same-local-day caveat:** the recording Member can edit a `Rejected` close only on the same branch-local day (§6.13 — "same rules as Draft"); after that day, a Manager/Admin must make the correction. **Gap §0.3.3-I:** this contradicts the §7.2 rhythm — managers review *yesterday's* closes, so in the normal flow the rejection arrives after the operator's window has already closed and this screen's job is impossible for its primary user.
- **Data shown**
...
- **Actions** — edit items → `PUT /dailyclose/{id}/items` (auto `Rejected → Draft`; the persisted variance row is kept and updated in place, §6.13); resubmit → `POST /dailyclose/{id}/submit`.
- **Audit / lock context** — the rejection reason stays in the audit trail; resubmission restamps `SubmittedAt`; the lock guard is unchanged. Denials mirror Close day: item edits → 409 `DAILYCLOSE_NOT_EDITABLE` (state, recorder, or day); submit → 409 `DAILYCLOSE_NOT_SUBMITTABLE` for state, 403 for the Member identity causes.
- **States** — *rejected* (reason leading) · *editing* (back in Draft) · *resubmitted* (awaiting) · *error*.
...
Surfaced while mapping screens to the pre-M7.5 contract; **all resolved** by [server Milestone 7.5 — Frontend UX Contract Gaps](../../server/docs/milestones.md), shipped at spec `v31` — the spec has since moved to `v32` with no change to these contracts (`loto-specs.md` §6.14). The per-field tags above have been updated to the shipped DTOs. Outcomes:
...
2. **Opening values are not returned.** Resolved (M7.5 Phase 2) **for closes that have items**: `GET /dailyclose/{dailyCloseId}/review` returns `ResponseDailyCloseReviewJson` — the close header plus per-item `OpeningValue?` (§6.5 server-derived; `null` on the variance row), `ClosingValue`, and `IsCashVarianceProduct`. Same read scope as reading the close itself, so the operator's Close-day form may use it too. **Caveat found in 0.3.3:** review projects only the close's *existing* items, so a fresh, just-opened close returns no rows — opening values are unavailable exactly when the operator first needs them. Reopened as gap §0.3.3-D below.
...
Unlike the resolved M7.5 batch above, these are open. **A–E** were surfaced by the 0.3.3 mapping; **F–T** by the 2026-07 catalog review (each verified against the server code, not just the docs). None blocks M1 (visual direction) — but **every screen named in the gap → screen map below is decision-blocked for M4 blueprints and the build**; the map is authoritative for the M4 gate (inline pointers in entries are convenience mirrors, not the source of truth). Decisions land through [server M7.7](../../server/docs/milestones.md) (M9.5 verification feeds the fiado one; server M8 owns C/N; design 0.4.5 owns P; design 0.4.1 owns R; server M11 owns Q); the catalog entries must be re-synced as each resolves.

**Phase 1 decisions — server M7.7, signed off by the dev team 2026-07-27; amended 2026-07-28 (reviewer follow-ups)** *(recorded here per the M7.7 Phase 1 contract; authoritative detail + implementation phases live in [server M7.7](../../server/docs/milestones.md). Each stays **decided, pending implementation** — the per-entry re-sync to resolved is M7.7 Phase 8.4):*

- **G → freeze at submit.** Submitting seals the `(account, date)` ledger; variance-affecting mutations are blocked while a non-`Draft` close exists. Explicit graduated undo — operator **Recall** for `Submitted`, manager **Reopen** for `Approved`; the silent `Submitted → Draft`-on-save is removed and submit is blocked while `Draft` rows exist.
...
- **I → operator edits a `Rejected` close until period-lock, and Manager/Admin may also correct it**; the same-day rule survives only for the voluntary `Submitted → Draft` recall.
- **J → add the `Notes` write path** (editable on the `Draft` `PUT /items` path, frozen at submit, max 1000), surfaced on Daily-close approval. Column kept.
...
> The individual gap entries below retain their original **question** framing for context. Where a gap appears in the decisions above (A · G · H · I · J · K · L · O · S), the decision above is authoritative and the entry's "decide / choose" language is superseded — the formal per-entry re-sync to *resolved* is M7.7 Phase 8.4.
...
- **D. Fresh-close opening values — `[gap]`.** `GET /dailyclose/{id}/review` projects only the close's existing items, and a just-opened close has none — so the sanctioned one-call form cannot show opening values until the first `PUT /items` lands. **The data is not unreachable.** A three-call interim composition exists today: `GET /product` (all rows) + `GET /dailyclose?AccountId` (pick the prior close) + `GET /dailyclose/{priorId}/review` (flagged item values — its non-variance `ClosingValue`s *are* today's openings). It is a gap rather than a blessed composition because the middle step re-implements the §6.5 selection rules (active-only, `Date DESC, CreatedAt DESC, Id DESC` tie-breaks, branch-local today, zero fallback) that M7.5 deliberately centralized server-side — duplicated in web and mobile, they can drift from the server and from each other. Candidate server fix, **no new route needed**: expand the existing review projection to enumerate all active products with `DisplayOrder`, server-derived `OpeningValue?`, a nullable not-yet-entered `ClosingValue`, and the system-product flag. Until then, either ship the interim composition (accepting the duplicated lookup, ideally as one `shared/` helper) or show openings only after the first save.
...
- **F. No pre-submit variance — `[gap]`, operator-critical.** §7.1 step 4 says the system "calculates/displays CashVariance" while the operator enters closing values (the legacy Access recalculated live), but the shipped contract computes the variance only on submit. The operator's core moment — counting the drawer — is feedback-blind: submit to find out you're short, then recall → fix → resubmit. Server fix dovetails with gap D: expand the review projection (or add a variance preview) to return a live §6.12 computation for a `Draft` close, using the **same calculator** as submit. Bundled in server M7.7.
- **G. Approved/submitted close variance can go stale — `[decision]`.** Transaction create / draft-finalize / cancel check only `LockDate` — never the close state of their `(account, date)`. Sequence: close submitted and approved → a draft is finalized or a row cancelled → the ledger moves but the persisted approved variance doesn't, and the stale number feeds the dashboard, cash-variance summary, and monthly reconciliation. The strategy must be **approval-preserving** (server M7.7 item 1.1): **(a)** freeze the account/day ledger at submit, with explicit reopen + re-approval, or **(b)** any variance-affecting mutation invalidates the close — an `Approved` close requires re-approval, and a **`Submitted`** close either blocks the mutation or returns to `Draft` for resubmission (the Submitted case is defined, not left implicit). Live recomputation on read surfaces may *supplement* either as a divergence display, but is not a standalone option — it would leave an `Approved` number nobody approved. Related tightening: outstanding `Draft` rows at submit time should block (or force a decision at) submission, not just warn. Decide before blueprinting the close/approval screens.
...
- **I. Rejected-close edit window defeats the review rhythm — `[decision]`.** A Member may edit a `Rejected` close only on the close's own branch-local day (§6.13), but §7.2 has managers reviewing *yesterday's* closes — so in the normal flow the rejection arrives after the window closed, and Fix & resubmit is impossible for its primary user. Preferred: the recording operator may edit an explicitly `Rejected` close until the period locks (the same-day rule stays for voluntary `Submitted → Draft` recall only). Alternative: prior-day corrections become manager-owned and §7.1's operator-resubmission promise is dropped.
- **J. `DailyClose.Notes` has no write path — `[gap]`.** §3.14 defines it ("Operator notes about the day", varchar 1000), every read DTO returns it, no request DTO can set it, no screen shows it. Add a write path (on submit or `PUT /items`) and surface it on Daily-close approval — an operator explaining "R$20 short, PIX terminal down 17h" is exactly what prevents the reject → phone-call → resubmit loop — or drop the column.
...
- **L. Terminal-only closes unenforced — `[gap]`.** `OpenDailyCloseUseCase` checks scope/workflow/lock but not `AccountType` — a close can be opened on a Tab or Bank account, though only terminals are expected closers and the product rows are drawer counts. Enforce Terminal on the server and in the Open-day account options. UX follow-on for M4: workstation-first framing — the operator picks a terminal once; fiado types auto-route to its paired Tab; the account switcher hides when there's only one. **Open sub-decision (server M7.7 item 1.8):** auto-routing to the paired Tab conflicts with explicit `OperatorAccount` scope — decide whether assigning a Terminal auto-assigns its Tab, implies Tab permission, or requires both assignments. The multi-terminal cockpit selector is a named M4 blueprint task. **Also unenforced: the open date.** `EnsureCanOpen` ignores its date argument entirely (verified) — a Member can open a close for an old or future date, then immediately cannot edit it (item edits require the same branch-local day). Server-side validation or inference of the open `Date` against branch-local today ships with the same M7.7 Phase 3 guards.
...
| D   | Close day                                                                                                                                                                                               |
| E   | Operators                                                                                                                                                                                               |
| F   | Close day · Fix & resubmit                                                                                                                                                                              |
| G   | Daily-close approval · Transaction — Create · Transaction — Edit · Installment plan · Close day · Operator Day Cockpit · Manager Work Queue · Cash-variance summary · Monthly reconciliation + lock     |
...
| I   | Fix & resubmit · Operator Day Cockpit · Daily-close approval                                                                                                                                            |
| J   | Close day · Daily-close approval                                                                                                                                                                        |
| K   | Monthly reconciliation + lock · Settings · Manager Work Queue                                                                                                                                           |
| L   | Open day · Operator Day Cockpit · Transaction — Create (account/Tab routing) · Account assignment                                                                                                       |
