...
- **Access** — the §6.13 edit-items matrix: for a plain Draft, the recording Member on the same branch-local day; for a Rejected correction or opening-recheck Draft, the recording operator until period lock; Manager/Admin anytime pre-lock. A direct prior-day Manager/Admin Recall/Reopen creates a Manager-owned plain Draft and does not grant Member access by inference.
...
- **Actions** — preview variance → `POST /dailyclose/{id}/variance-preview {Items}`; save values/Notes → `PUT /dailyclose/{id}/items {Version, Notes?, Items}` → `ResponsePutDailyCloseItemsJson {DailyClose, AffectedSuccessor?}`. A real counted-input/first-count change may return exactly the next eligible later official close moved to Draft; show *“O fechamento de {data} voltou para rascunho porque o fechamento anterior foi corrigido.”* Notes-only/no-op/reordered saves return no affected successor. **Submit** → `POST /dailyclose/{id}/submit` (atomically checks recorded count, Draft rows, and intervening uncounted activity, then computes/persists variance and freezes the ledger). On a mistaken Submitted close, **Desfazer envio** → `POST /dailyclose/{id}/recall`. Ordinary `PUT /items` on Submitted always returns `DAILYCLOSE_NOT_EDITABLE`; a recording Member may undo the send only on the same local day, while Manager/Admin may do so pre-lock.
- **Audit / lock context** — `PUT /items` uses the close's PostgreSQL `xmin`-backed `Version`; stale saves return 409 `DAILYCLOSE_STALE_WRITE` and the form reloads before retrying. Every transition stamps `UpdatedAt`/`UpdatedByUserId`; Submit stamps `SubmittedAt`, clears opening-recheck lineage, freezes Notes and the account/day ledger atomically; Desfazer envio clears `SubmittedAt`; all transitions are blocked at/before `LockDate`. A busy account-history coordination wait returns retryable 409 `DAILYCLOSE_LEDGER_COORDINATION_BUSY` rather than an unknown error.
...
- **Access** — the recording operator may edit an explicitly `Rejected` close, its correction Draft, or an opening-recheck Draft until the period locks; Manager/Admin may also correct it. Same-day Member restriction remains only for voluntary **Desfazer envio** of Submitted. A direct prior-day elevated Recall/Reopen is Manager-owned.
- **Data shown**
...
Surfaced while mapping screens to the pre-M7.5 contract; **all resolved** by [server Milestone 7.5 — Frontend UX Contract Gaps](../../server/docs/milestones.md), shipped at spec `v31` — committed `HEAD` has since moved to `v34`; M7.7 Phase 2's review expansion supersedes the caveat noted in outcome 2. M7.7 Phase 3's expanded integrity work is still pending and remains a gate below. The per-field tags above have been updated to the current DTOs. Outcomes:
...
## Backend contract gaps — signed for server M7.7 Phase 3, not yet shipped

The original Phase 3 implementation exists only in the dirty worktree and has not shipped from committed `HEAD`. Its independent review reopened the close-chain portion of G and added response/coordination hardening. The signed 2026-08-11 amendment is absorbed into the same unreleased Phase 3; these entries remain M4-gated until the combined implementation and spec sync land:

- **G. Submitted/approved variance integrity — signed, pending.** Submit freezes same-day ledger mutations, and a real counted-item change on an earlier close explicitly returns the next eligible opening-chain successor to Draft for resubmission/reapproval. Account-wide database coordination, recorded-count eligibility, Terminal activity/open-close binding, uncounted prior-activity Submit blocking, bounded waits/cancellation, and Draft CashVariance suppression are all part of the one Phase 3 delivery.
- **I. Rejected/cascade correction window — signed, pending.** The recording operator may edit and resubmit an explicitly Rejected or opening-recheck-demoted close until `LockDate`; Manager/Admin may also correct it. Direct prior-day elevated Recall/Reopen is Manager-owned. Same-day Member restriction applies only to voluntary Recall of Submitted.
- **L (server open guards) — signed, pending.** DailyClose opens become Terminal-only; Member opens are branch-local today only, Manager/Admin may backdate before lock, and every role rejects future dates. Terminal activity must have an open close first. The later Phase 5/M4 remainder is workstation/paired-Tab routing and assignment UX.

---

## Gaps & notes surfaced by 0.3.3 — open

These are the remaining open entries after M7.5 and M7.7 Phase 2's D/F/J close-out. M7.7 Phase 3's G/I and DailyClose-open half of L are signed but unshipped, so they remain in this register and in the authoritative gap → screen map. None blocks M1 (visual direction), but **every screen named below is implementation-blocked for M4 blueprints and the build**. Decisions land through [server M7.7](../../server/docs/milestones.md) (M9.5 verification feeds the fiado one; server M8 owns C/N; design 0.4.5 owns P; design 0.4.1 owns R; server M11 owns Q); the catalog entries are re-synced only when implementation lands.

**Phase 1 decisions — server M7.7, signed off by the dev team 2026-07-27; amended 2026-07-28 and 2026-08-11 (reviewer follow-ups)** *(authoritative detail + implementation phases live in [server M7.7](../../server/docs/milestones.md). J shipped in Phase 2; G/I and L's open guards remain pending the expanded Phase 3; the other entries remain pending their implementation phases):*

- **G → freeze at submit + explicit opening-chain invalidation — signed, pending Phase 3.** Submitting seals same-day ledger movement. A later official close whose opening genuinely changes because an earlier counted close is corrected is explicitly returned to Draft with an opening-recheck marker; no official value is recomputed silently. Terminal writes require an open close, Submit rejects never-counted/current or intervening uncounted-activity days, and account-wide coordination serializes close/ledger history with bounded retryable failure.
...
- **I → operator edits a `Rejected` or opening-recheck-demoted close until period-lock, and Manager/Admin may also correct — signed, pending Phase 3**; the same-day rule survives only for voluntary Member Recall, while direct prior-day elevated Recall/Reopen is Manager-owned.
...
> The individual open gap entries below retain their original **question** framing for context. For decided but unshipped entries (A · G · H · I · K · L · O · S), the signed decision above is authoritative and any "decide / choose" wording is historical context pending implementation.
...
- **G. Submitted/approved close variance can go stale — `[signed, pending Phase 3]`.** The original account/day freeze protects ledger writes but not the cross-day opening dependency: editing an earlier counted close can change a later official close's opening. The expanded Phase 3 keeps freeze-at-submit and adds explicit one-node opening-chain demotion, first-save counted eligibility, account-wide coordination, Submit prerequisites, Terminal activity binding, bounded cancellation-aware waits, and persisted-snapshot suppression on every Draft response. Close day, approval, dashboard, cash-variance, monthly reconciliation, transaction create/edit/installment, and Manager Work Queue remain gated until it ships.
- **I. Rejected/opening-recheck correction ownership — `[signed, pending Phase 3]`.** The recording operator can correct an explicitly Rejected or cascade-demoted close until period lock. A direct prior-day Manager/Admin Recall/Reopen is Manager-owned; the same-day Member rule remains only for voluntary Recall of Submitted. The §6.13 matrix and UI copy must keep Reject, Desfazer envio, Reopen, and opening recheck distinct.
...
| E   | Operators                                                                                                                                                                                               |
| G   | Close day · Daily-close approval · Manager Work Queue · Dashboard · Cash-variance summary · Monthly reconciliation · Transaction — Create/Edit · Installment plan                                    |
...
| I   | Close day · Daily-close approval · Manager Work Queue                                                                                                                                                   |
