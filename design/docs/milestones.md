...
**Precondition:** M3 + M0's baselined `screens.md` + M1's direction. **Per-screen gate:** a blueprint for any screen named in the catalog's **gap → screen map** (the authoritative table in `screens.md`'s gaps section — inline pointers are convenience mirrors) requires that gap's decision resolved and the entry re-synced first. Today the map spans the close/approval flow (G/I; D/F/J resolved and re-synced in server M7.7 Phase 2), month lock (K), the fiado screens (A/H), Open day + cockpit (L), Transaction Create/Edit + Installments (G/M), the Work Queue (P), Accounts (S), Settings + time screens (K/O/T), and the future auth/report screens (C/N/Q); gap R gates the nav shell.
