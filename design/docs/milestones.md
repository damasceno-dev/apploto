...
- [ ] **0.4.1** Add the **navigation map / IA** into `screens.md`: how screens link, the branch-session gate, role-based nav differences (Member vs Manager/Admin) — including the **manager-as-operator dual-mode decision** (gap R: both homes exposed / explicit mode toggle / cockpit in manager nav).
...
**Precondition:** M3 + M0's baselined `screens.md` + M1's direction. **Per-screen gate:** a blueprint for any screen named in the catalog's **gap → screen map** (the authoritative table in `screens.md`'s gaps section — inline pointers are convenience mirrors) requires that gap's implementation has landed and the entry has been re-synced. D/F/J landed in server M7.7 Phase 2. G/I and the DailyClose-open half of L are signed but remain gated on the expanded, unreleased M7.7 Phase 3 (same-day freeze plus explicit opening-chain invalidation, counted-source eligibility, Terminal activity binding, and bounded account-wide coordination). The remaining map spans month lock (K), the fiado screens (A/H), cockpit/account routing (L), Transaction Edit + Installments (M), the Work Queue (P), Accounts (S), Settings + time screens (K/O/T), and the future auth/report screens (C/N/Q); gap R gates the nav shell.

**Seeded blueprint backlog** (named tasks the review already identified): the multi-terminal cockpit selector + workstation-first Open-day framing (gap L); the guided Add/Offboard-employee journey (0.4.6); the manager-as-operator mode switch (gap R).
