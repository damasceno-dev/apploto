...
- [x] **0.4.1** Add the **navigation map / IA** into `screens.md`: how screens link, the branch-session gate, role-based nav differences (Member vs Manager/Admin) — including the **manager-as-operator dual-mode decision** (gap R: explicit **Gestão ↔ Meu turno** switch for linked Manager/Admin; management-only setup explanation when unlinked; mode changes navigation/home context, not token role or server permissions).
- [ ] **0.4.2** Add the shared **cross-screen patterns** into `screens.md` (semantically): error display, list pagination, empty/loading conventions, auth-tier gating, **session/token expiry** (silent-renew failure → re-login → return-to-context; branch-token drop on switch), **server-authoritative branch date/time**, **idempotent submits / uncertain-save-outcome handling**, unsaved-changes + concurrency conventions (decided: unsaved input is always kept on the device and restored on return; Close day additionally autosaves to the server on today's plain Draft — `screens.md` Conventions + Close day), the explicit **online-only vs offline** decision (an online-only MVP is acceptable — make it a decision, not an accident), and **screen-level accessibility patterns** (focus order, touch targets, error announcement). **Server dependency delivered in M7.7 Phase 6 (spec v39):** branch time comes from `GET /branch/current`; transaction/installment creates use persisted 24-hour `Idempotency-Key` replay; guarded mutations use strong quoted-decimal `If-Match`/ETag PostgreSQL `xmin` with verbatim stale 409 keys. This design item remains unchecked until the complete pattern catalog is authored; it now consumes those exact contracts rather than inventing them. These seed M3/M4.
- [ ] **0.4.3** Add `Synced against loto-specs: vNN` markers to `product.md` + `screens.md`; mark both **Reviewed baseline** — a close-out with explicit *decision-blocked* entries (every screen named in the catalog's **gap → screen map** stays draft until its gap's decision lands and the entry is re-synced; the map is authoritative, inline pointers are mirrors), **not** a claim that all semantics are final. M1 may now start. Requires 0.4.5 done (gap P is Work-Queue semantics, not visuals).
- [ ] **0.4.4** Reconcile `product.md` with the 2026-07 review findings: the identity-tier "profile" promise has no cataloged Profile screen (gap N — profile / logout / password recovery are uncataloged and unserved by the contract, now scoped into server M8); clarify that **mobile clones web** means the same screen *set and semantics*, with platform-appropriate interaction patterns.
- [x] **0.4.5** Resolve **gap P** in the catalog before the 0.4.3 baseline: define the Work Queue default-day rule (no silently borrowed due-date business-day convention; whether Saturdays/Sundays count as operating days is itself part of the decision — the system has no universal operating calendar), the cross-date pending-backlog presentation, and the Draft-count composition (scope to the earliest unlocked month and drop the extra call, or define its range). **Done:** today is literal and never switched automatically; a pending-backlog banner covers earlier unlocked days; the Draft count spans every unlocked day up to today (both served by the dashboard aggregate — gap W, server M7.7 Phase 7.5); the queue's group order is fixed — gap P is resolved in `screens.md`.
- [ ] **0.4.6** *(deferred, owned — executes when its dependencies land)* Catalog the **auth & account screen set**: Invite accept, Registration-via-invite, Forgot/Reset password, Profile, Change password, Logout/session menu, **Continuar com Google** on Login and Invite accept (which offers **Criar senha** or **Continuar com Google**), the internal **Plataforma** area with the "Nova lotérica" onboarding wizard, and the owner **setup checklist** on Manager Work Queue — onboarding is operated (gap C, decided in server M8), so there is no public Register or Create-branch screen — plus the **guided Add employee / Offboard employee flows** (membership + operator link + account assignment in one journey; the Operators screen's "under a minute" promise currently spans three screens; a Member invitation may pre-select the operator, so accepting it creates the link). Depends on **server M8** (auth contract) and **server M7.7** decision 1.8 (Terminal↔Tab assignment pairing). Gap E is resolved in M7.7 Phase 6: removing branch membership atomically clears matching Operator login links, so the employee flows no longer need to invent stale-link display behavior.
...
7. **Review feedback on the M0 clickable prototype:** keep corners restrained (≤ 4px), prefer hairline dividers to boxed cards, and avoid pill-shaped or heavily tinted highlight cards — they read as improvised rather than finance-grade.
...
**Precondition:** M3 + M0's baselined `screens.md` + M1's direction. **Per-screen gate:** a blueprint for any screen named in the catalog's **gap → screen map** (the authoritative table in `screens.md`'s gaps section — inline pointers are convenience mirrors) requires that gap's implementation has landed and the entry has been re-synced. A parenthetical feature qualifier on a map row limits the gate to that feature; the rest of that screen's blueprint proceeds. D/F/J shipped in server M7.7 Phase 2; G, I, and L's server open guards in Phase 3; K in Phase 4; B, E, M, and T in Phase 6; O in Phase 7. The remaining map spans Close day's fiado reference (A), the fiado screens with Client statement and Receber pagamento (H), paired-Tab routing on Transaction — Create and Account assignment (L), Accounts (S), the future auth/report screens (C/N/Q), the self-recorded marker and own-row guard (U, server M8), the operator-link request surfaces (V, server M8), and the Work Queue backlog banner and Draft count (W, server M7.7 Phase 7.5). Gap P is resolved by 0.4.5. Gap R is resolved by 0.4.1 and no longer gates the navigation shell.

**Seeded blueprint backlog** (named tasks the review already identified): the multi-terminal cockpit selector + workstation-first Open-day framing (semantics now in the catalog; M4 owns the visual blueprint); the guided Add/Offboard-employee journey, the Plataforma onboarding wizard, and the owner setup checklist (0.4.6). The manager-as-operator mode switch semantics are resolved in 0.4.1; M4 owns only its visual blueprint.
