...
- [ ] **0.4.5** Resolve **gap P** in the catalog before the 0.4.3 baseline: define the Work Queue default-day rule (no silently borrowed due-date business-day convention; whether Saturdays/Sundays count as operating days is itself part of the decision — the system has no universal operating calendar), the cross-date pending-backlog presentation, and the Draft-count composition (scope to the earliest unlocked month and drop the extra call, or define its range).
- [ ] **0.4.6** *(deferred, owned — executes when its dependencies land)* Catalog the **auth & account screen set**: Invite accept, Registration-via-invite, Forgot/Reset password, Profile, Change password, Logout/session menu — plus the **guided Add employee / Offboard employee flows** (membership + operator link + account assignment in one journey; the Operators screen's "under a minute" promise currently spans three screens). Depends on **server M8** (auth contract) and **server M7.7** decision 1.8 (Terminal↔Tab assignment pairing). Gap E is resolved in M7.7 Phase 6: removing branch membership atomically clears matching Operator login links, so the employee flows no longer need to invent stale-link display behavior.
...
**Precondition:** M3 + M0's baselined `screens.md` + M1's direction. **Per-screen gate:** a blueprint for any screen named in the catalog's **gap → screen map** (the authoritative table in `screens.md`'s gaps section — inline pointers are convenience mirrors) requires that gap's implementation has landed and the entry has been re-synced. A parenthetical feature qualifier on a map row limits the gate to that feature; the rest of that screen's blueprint proceeds. D/F/J landed in server M7.7 Phase 2. G/I and the DailyClose-open half of L are signed but remain gated on the expanded, unreleased M7.7 Phase 3 (same-day freeze plus explicit opening-chain invalidation, counted-source eligibility, Terminal activity binding, and bounded account-wide coordination). The remaining map spans month lock (K), the fiado screens (A/H), cockpit/account routing (L), Transaction Edit + Installments (M), the Work Queue (P), Accounts (S), Settings + time screens (K/O/T), the future auth/report screens (C/N/Q), and Daily-close approval's self-recorded marker only (U, no owning milestone yet). Gap R is resolved by 0.4.1 and no longer gates the navigation shell.

**Seeded blueprint backlog** (named tasks the review already identified): the multi-terminal cockpit selector + workstation-first Open-day framing (gap L); the guided Add/Offboard-employee journey (0.4.6). The manager-as-operator mode switch semantics are resolved in 0.4.1; M4 owns only its visual blueprint.
