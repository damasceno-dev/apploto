...
- [ ] **0.4.2** Add the shared **cross-screen patterns** into `screens.md` (semantically): error display, list pagination, empty/loading conventions, auth-tier gating, **session/token expiry** (silent-renew failure → re-login → return-to-context; branch-token drop on switch), **server-authoritative branch date/time**, **idempotent submits / uncertain-save-outcome handling**, unsaved-changes + concurrency conventions, the explicit **online-only vs offline** decision (an online-only MVP is acceptable — make it a decision, not an accident), and **screen-level accessibility patterns** (focus order, touch targets, error announcement). The three server-owned contracts here (branch time, idempotency keys, concurrency/stale-state) are milestoned in **server M7.7 Phase 6** — this item consumes them, it does not invent them. These seed M3/M4.
